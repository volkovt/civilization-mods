...
def main() -> None:
    ...
    require_count(cursor, "active_power_promotions", "SELECT COUNT(*) FROM UnitPromotions WHERE Type IN ('PROMOTION_SAYAJIN_INSTANT_TRANSMISSION','PROMOTION_SAYAJIN_FINAL_EXPLOSION') AND CannotBeChosen=1 AND LostWithUpgrade=0", 2)
    require_count(cursor, "goku_teleport_forms", "SELECT COUNT(*) FROM Unit_FreePromotions WHERE PromotionType='PROMOTION_SAYAJIN_INSTANT_TRANSMISSION' AND UnitType IN ('UNIT_SAYAJIN_HERO_GOKU_MEDIEVAL','UNIT_SAYAJIN_HERO_GOKU_RENAISSANCE','UNIT_SAYAJIN_HERO_GOKU_INDUSTRIAL','UNIT_SAYAJIN_HERO_GOKU_MODERN','UNIT_SAYAJIN_HERO_GOKU_POSTMODERN','UNIT_SAYAJIN_HERO_GOKU_FUTURE')", 6)
    require_count(cursor, "vegeta_explosion_forms", "SELECT COUNT(*) FROM Unit_FreePromotions WHERE PromotionType='PROMOTION_SAYAJIN_FINAL_EXPLOSION' AND UnitType IN ('UNIT_SAYAJIN_HERO_VEGETA_MEDIEVAL','UNIT_SAYAJIN_HERO_VEGETA_RENAISSANCE','UNIT_SAYAJIN_HERO_VEGETA_INDUSTRIAL','UNIT_SAYAJIN_HERO_VEGETA_MODERN','UNIT_SAYAJIN_HERO_VEGETA_POSTMODERN','UNIT_SAYAJIN_HERO_VEGETA_FUTURE')", 6)
    require_count(cursor, "active_power_leaks", "SELECT COUNT(*) FROM Unit_FreePromotions WHERE (PromotionType='PROMOTION_SAYAJIN_INSTANT_TRANSMISSION' AND UnitType NOT LIKE 'UNIT_SAYAJIN_HERO_GOKU_%') OR (PromotionType='PROMOTION_SAYAJIN_FINAL_EXPLOSION' AND NOT (UnitType='UNIT_SAYAJIN_HERO' OR UnitType LIKE 'UNIT_SAYAJIN_HERO_VEGETA_%'))", 0)
    require_count(cursor, "premature_active_powers", "SELECT COUNT(*) FROM Unit_FreePromotions WHERE PromotionType IN ('PROMOTION_SAYAJIN_INSTANT_TRANSMISSION','PROMOTION_SAYAJIN_FINAL_EXPLOSION') AND (UnitType IN ('UNIT_SAYAJIN_HERO','UNIT_SAYAJIN_HERO_GOKU') OR UnitType LIKE '%_CLASSICAL')", 0)
    ...
    require_count(cursor, "melee_laser_projectiles", "SELECT COUNT(*) FROM ArtDefine_UnitMemberCombatWeapons WHERE (UnitMemberType='ART_DEF_UNIT_MEMBER_SAYAJIN_VEGETA' OR UnitMemberType LIKE 'ART_DEF_UNIT_MEMBER_SAYAJIN_VEGETA_%' OR UnitMemberType='ART_DEF_UNIT_MEMBER_SAYAJIN_BROLY' OR UnitMemberType LIKE 'ART_DEF_UNIT_MEMBER_SAYAJIN_BROLY_%') AND [Index]=0 AND ID='ART_DEF_VEFFECT_TRAIL_RAILGUN_PROJ' AND ProjectileSpeed>0", 16)
    require_count(cursor, "melee_unused_weapon_slots", "SELECT COUNT(*) FROM ArtDefine_UnitMemberCombatWeapons WHERE (UnitMemberType='ART_DEF_UNIT_MEMBER_SAYAJIN_VEGETA' OR UnitMemberType LIKE 'ART_DEF_UNIT_MEMBER_SAYAJIN_VEGETA_%' OR UnitMemberType='ART_DEF_UNIT_MEMBER_SAYAJIN_BROLY' OR UnitMemberType LIKE 'ART_DEF_UNIT_MEMBER_SAYAJIN_BROLY_%') AND [Index]<>0", 0)
